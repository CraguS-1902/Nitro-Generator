...
class NitroGen:
    ...
    def main(self):
        ...
        if os.name == "nt":
            print("")
            ctypes.windll.kernel32.SetConsoleTitleW("Nitro Generator and Checker - Made by CraguS#7499") 
        else: 
            print(f'\33]0;Nitro Generator and Checker - Made by CraguS#7499\a', end='', flush=True) 
...
""")
        time.sleep(2) 
        self.slowType("Made by: CraguS#7499", .02) 
        time.sleep(1) 
        self.slowType("\nInput How Many Codes to Generate and Check: ", .02, newLine = False) 

        num = int(input('')) 
        if num <= 0:
            self.slowType("\nPlease type a Number above 0", .02, newLine = False) 
            time.sleep(2)
            Gen = NitroGen()
            Gen.main()
        ...
        for i in range(num): 
            ...
            if os.name == "nt": 
                ctypes.windll.kernel32.SetConsoleTitleW(f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by CraguS#7499") 
                print("")
            else:
                print(f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by CraguS#7499\a', end='', flush=True) 
